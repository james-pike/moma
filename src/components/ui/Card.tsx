...
const Root = component$<PropsOf<'div'>>((props) => {
  return (
    <div
      {...props}
      class={cn(
        'rounded-base p-2 border mb-0.5 border-white dark:border-gray-700 dark:bg-gray-800 bg-gray-100 text-card-foreground shadow-sm',
        props.class,
      )}
    >
      <Slot />
...
const Content = component$<PropsOf<'div'>>((props) => {
  return (
    <div {...props} class={cn('px-2 pt-2 pb-1', props.class)}>
      <Slot />
    </div>
  );
});
